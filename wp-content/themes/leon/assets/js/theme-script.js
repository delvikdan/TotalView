(function($){
	...
	CherryJsCore.theme_script = {
		...
		smart_slider_init: function( self ) {
			$( '.leon-smartslider' ).each( function() {
				var slider = $(this),
		...
		swiper_carousel_init: function ( self ) {
			...
			jQuery('.leon-carousel').each( function() {
				var swiper = null,
		...
		subscribe_init: function( self ) {
			CherryJsCore.variable.$document.on( 'click', '.subscribe-block__submit', function( event ){
				...
				var $this       = $(this),
					...
					nonce      = form.find( 'input[name="leon_subscribe"]' ).val(),
		...
		main_menu: function ( self, $mainNavigation ) {
			...
			if ( hasprop.call( window, 'leon' ) &&
			     hasprop.call( window.leon, 'more_button_options' ) &&
			     hasprop.call( window.leon.more_button_options, 'more_button_type' ) ) {
				switch( window.leon.more_button_options.more_button_type ) {
					case 'image':
						imgurl = window.leon.more_button_options.more_button_image_url;
						if ( window.leon.more_button_options.retina_more_button_image_url ) {
							srcset = ' srcset="' + window.leon.more_button_options.retina_more_button_image_url + ' 2x"';
						}
						moreMenuContent = '<img src="' + imgurl + '"' + srcset + ' alt="' + moreMenuContent + '">';
						hasimg = true;
					break;
					case 'icon':
						moreMenuContent = '<i class="fa ' + window.leon.more_button_options.more_button_icon + '"></i>';
						hasicon = true;
					break;
					case 'text':
					default:
						moreMenuContent = window.leon.more_button_options.more_button_text || moreMenuContent;
						hasimg = false;
						hasicon = false;
					break;
				}
...
var widget_layers_widget_slide_3_slider = jQuery('#huilo').swiper({
...
jQuery(function() {
  jQuery('a[href*=#]').on('click', function(e) {
    e.preventDefault();
    jQuery('html, body').animate({ scrollTop: jQuery(jQuery(this).attr('href')).offset().top-140}, 500, 'linear');
  });
});
...
jQuery(window).scroll(function(){
   if(jQuery(window).scrollTop()<20){
         jQuery('#scrolldown').stop(true,true).fadeIn("slow");
   } else {
         jQuery('#scrolldown').stop(true,true).fadeOut("slow");
   }
});
