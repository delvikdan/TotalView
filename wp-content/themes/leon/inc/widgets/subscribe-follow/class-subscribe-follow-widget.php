...
class Leon_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'leon' );
		$this->widget_id          = 'leon_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'leon' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'leon' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'leon' ),
				'label' => esc_html__( 'Subscribe Title', 'leon' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'leon' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your e-mail', 'leon' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'leon' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'leon' ),
				'label' => esc_html__( 'Subscribe submit label', 'leon' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'leon' ),
				'label' => esc_html__( 'Subscribe success', 'leon' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'leon' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'leon' ),
				'label' => esc_html__( 'Follow Title', 'leon' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'leon' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'leon' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'leon' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'leon' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'leon' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'leon' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'leon' ),
					'top-center'    => esc_html__( 'Top Center', 'leon' ),
					'top-right'     => esc_html__( 'Top Right', 'leon' ),
					'center-left'   => esc_html__( 'Middle Left', 'leon' ),
					'center'        => esc_html__( 'Middle Center', 'leon' ),
					'center-right'  => esc_html__( 'Middle Right', 'leon' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'leon' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'leon' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'leon' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'leon' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'leon' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'leon' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'leon' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'leon' ),
					'contain' => esc_html__( 'Contain', 'leon' ),
					'auto'    => esc_html__( 'Auto', 'leon' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'leon' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_leon_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_leon_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'leon' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'leon' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'leon' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'leon' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'leon' );
		}
	...
	public function get_social_nav() {
		return leon_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = leon_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'leon_subscribe' ) ) {
...
add_action( 'widgets_init', 'leon_register_subscribe_follow_widgets' );
function leon_register_subscribe_follow_widgets() {
	register_widget( 'Leon_Subscribe_Follow_Widget' );
}
