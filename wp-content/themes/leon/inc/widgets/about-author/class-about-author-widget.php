 ...
 * @package Leon
 */
if ( ! class_exists( 'Leon_About_Author_Widget' ) ) {

	class Leon_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'leon widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'leon' );
			$this->widget_id          = 'leon_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'leon' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'leon' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'leon' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'leon' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'leon' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'leon' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'leon' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'leon' ),
					'label' => esc_html__( 'Link label', 'leon' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'leon' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'leon-author-avatar' );
...
add_action( 'widgets_init', 'leon_register_about_author_widgets' );
function leon_register_about_author_widgets() {
	register_widget( 'Leon_About_Author_Widget' );
}
