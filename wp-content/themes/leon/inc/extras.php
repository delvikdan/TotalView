 ...
 * @return string
 */
 function leon_set_post_meta_value( $value ) {
 	$queried_obj = apply_filters( '_tm_queried_object_id', false );
 	if ( ! $queried_obj ) {
 	...
 	$curr_opions = 'leon_' . str_replace( 'theme_mod_', '', current_filter() );
  ...
  * Sidebar position
  */
 add_filter( 'theme_mod_sidebar_position', 'leon_set_post_meta_value' );

 /**
  * Header container type
  */
 add_filter( 'theme_mod_header_container_type', 'leon_set_post_meta_value' );

 /**
  * Content container type
  */
 add_filter( 'theme_mod_content_container_type', 'leon_set_post_meta_value' );
  /**
  * Footer container type
  */
 add_filter( 'theme_mod_footer_container_type', 'leon_set_post_meta_value' );
 ...
 * @return string
 */
function leon_render_macros( $string ) {

	$macros = apply_filters( 'leon_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function leon_render_icons( $content ) {
	$icons     = leon_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'leon_render_icons_callback', $content );
}
 ...
 * @return string
 */
function leon_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function leon_get_render_icons_set() {
	return apply_filters( 'leon_render_icons_set', array(
 ...
 * @return string
 */
function leon_render_theme_url( $url ) {
 ...
 */
function leon_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function leon_post_formats_gallery() {
	$size = leon_post_thumbnail_size();
	...
	$images = leon_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
 ...
 * @return bool
 */
function leon_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, leon_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function leon_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'post-thumbnail',
		'fullwidth'    => 'leon-thumb-l',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', leon_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', leon_theme()->customizer->get_default( 'blog_featured_image' ) );
